    ...
    <div className={`md:hidden sticky top-20 z-30  transition-all duration-700 ease-in-out opacity-100 bg-purple-900/90 text-white border-b-2 border-orange-500 flex justify-around p-4 ${sidebarButtonClicked ? 'opacity-0' : 'opacity-100'}`}>
      <Fade duration={700} className=''>
        <div className='grid grid-cols-2 xsm:grid-cols-3 sm:grid-cols-6 gap-x-4 gap-y-4 font-Charm text-xl'>
          <NavbarButton
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[0]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[0]}
          />
          <NavbarButton
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[1]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[1]}
          />
          <NavbarButton
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[2]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[2]}
          />
          <NavbarButton
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[3]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[3]}
          />
          <NavbarButton
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[4]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[4]}
          />
          <NavbarButton
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[5]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[5]}
          />
          <NavbarButton
            extraClasses={'col-span-2 xsm:col-span-3 sm:col-span-6'}
            myOnClick={() => {
              scrollToSection(otherData.navbarOptionLinks[6]);
              handleButtonClick()
            }}
            buttonName={otherData.navbarOptions[6]}
          />
        </div>
      </Fade>
    </div>
